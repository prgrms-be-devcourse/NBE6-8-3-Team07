...
const FairytaleList = () => {
  ...
  const handleToggleBookmark = async (
    ...
    try {
      ...
      const response = await customFetch(
        `${NEXT_PUBLIC_API_BASE_URL}/bookmark/${fairytaleId}`,
        {
          method: method,
      ...
      if (!response.ok) {
        throw new Error(
          `즐겨찾기 ${currentBookmarkStatus ? "해제" : "추가"} 실패! status: ${
            response.status
          }`
        );
      }
  ...
  const handleToggleVisibility = async (
    fairytaleId: number,
    currentVisibilityStatus: boolean
  ) => {
    ...
    try {
      const response = await customFetch(
        `${NEXT_PUBLIC_API_BASE_URL}/fairytales/${fairytaleId}/visibility?isPublic=${!currentVisibilityStatus}`,
        {
          method: "PATCH",
          credentials: "include",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `공개/비공개 설정 실패! status: ${response.status} - ${errorText}`
        );
      }
      ...
      alert(
        `동화가 ${
          !currentVisibilityStatus ? "공개" : "비공개"
        }로 설정되었습니다.`
      );
    } catch (error: unknown) {
  ...
  const handleDeleteFairytale = async (id: number, title: string) => {
    ...
    try {
      const response = await customFetch(
        `${NEXT_PUBLIC_API_BASE_URL}/fairytales/${id}`,
        {
          method: "DELETE",
          credentials: "include", // 인증 정보 포함
        }
  ...
  useEffect(() => {
    const fetchFairyTalesWithBookmarks = async () => {
      ...
      try {
        // 동화 목록 가져오기
        const fairytalesResponse = await customFetch(
          `${NEXT_PUBLIC_API_BASE_URL}/fairytales`,
          {
            credentials: "include", // 인증 정보 포함
          }
        );
        ...
        try {
          const bookmarksResponse = await customFetch(
            `${NEXT_PUBLIC_API_BASE_URL}/bookmarks`,
            {
              credentials: "include",
            }
          );
          ...
          if (bookmarksResponse.ok) {
            ...
            bookmarkedIds = bookmarksData.map(
              (bookmark: { fairytaleId: number }) => bookmark.fairytaleId
            );
          }
        } catch (bookmarkError) {
          console.warn(
            "즐겨찾기 정보를 가져오는데 실패했습니다:",
            bookmarkError
          );
        ...
        // 동화 목록에 즐겨찾기 상태 추가
        const fairytalesWithBookmarks: FairytaleWithBookmark[] =
          fairytalesData.map((tale) => ({
            ...tale,
                ...
                {filteredFairyTales.map((tale) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 whitespace-nowrap">
                      <button
                        onClick={() =>
                          handleToggleBookmark(tale.id, tale.isBookmarked)
                        }
                    ...
                    <td className="py-4 px-6 whitespace-nowrap">
                      <button
                        onClick={() =>
                          handleToggleVisibility(tale.id, tale.isPublic)
                        }
                    ...
                    <td className="py-4 px-6 whitespace-nowrap">
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          onClick={() =>
                            handleDeleteFairytale(tale.id, tale.title)
                          }
